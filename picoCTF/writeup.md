...
バイナリファイルをダウンロード。  
fileコマンドでは実行ファイルではなさそう。  
stirlingでバイナリを見てみる。  

先頭がBMなのでbmp画像。  
bmpのファイルフォーマットになぞらえてバイナリを読んでみる。  
...
00 00：予約領域 0
00 00：予約領域 0
BA D0 00 00：ファイル先頭から画像データまでのオフセット
BA D0 00 00：ヘッダサイズ。Windowなら40(0x28)なのに、大きすぎる。
```
0x28 00 00 00に変更して、拡張子をbmpに変更すると画像が表示されるが、フラグははずれらしい。  

さらにバイナリを読み進める。
```
6E 04 00 00：画像の幅。0x046E=1134ピクセル
32 01 00 00：画像の高さ。0x0132=306ピクセル
```
4:3に直してみる。  1134/4*3=859=0x351= 51 01 00 00  がダメ。  
微調整して、0x340にすると元画像が表示されフラグゲット。  
